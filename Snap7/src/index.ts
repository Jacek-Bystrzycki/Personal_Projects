...
const main = async (): Promise<void> => {
  ...
  let readTags: snap7.MultiVarRead[] = await createS7ReadTags(tagFile, 4);
  let writeTags: snap7.MultiVarWrite[] = await createS7WriteTags(tagFile, 5);
  const plc1: S7_Definition = new S7_Definition('10.0.0.15', 0, 1, readTags, writeTags);

  tagFile = 's7-tags-s1-p2.xlsx';
  readTags = await createS7ReadTags(tagFile, 6);
  writeTags = await createS7WriteTags(tagFile, 5);
  const plc2: S7_Definition = new S7_Definition('10.0.0.17', 0, 1, readTags, writeTags);
  ...
  readTags = await createS7ReadTags(tagFile, 4);
  writeTags = await createS7WriteTags(tagFile, 5);
  ...
  readTags = await createS7ReadTags(tagFile, 4);
  writeTags = await createS7WriteTags(tagFile, 5);
  const plc4: S7_Definition = new S7_Definition('10.0.0.10', 0, 1, readTags, writeTags);
