...
export const searchQueueForDone = (id: string, queue: S7_SyncQuery[] | MB_SyncQuery[] | UA_SyncQuery[]): boolean => {
  const findQuery = queue.find((query) => query.queryId === id);
  return findQuery?.isDone === true;
};

export const searchQueueForError = (id: string, queue: S7_SyncQuery[] | MB_SyncQuery[] | UA_SyncQuery[]): boolean => {
  const findQuery = queue.find((query) => query.queryId === id);
  return findQuery?.isError === true;
};

export const searchQueueForErrorMsg = (id: string, queue: S7_SyncQuery[] | MB_SyncQuery[] | UA_SyncQuery[]): string => {
  const findQuery = queue.find((query) => query.queryId === id);
